...
def prepare_for_coco_bquad_bbox(predictions, dataset):
    # assert isinstance(dataset, COCODataset)
    coco_results = []
    for image_id, prediction in enumerate(predictions):
        original_id = dataset.id_to_img_map[image_id]
        ...
        bquads = prediction.get_field("bquad").tolist()

        boxes = []
        for bquad in bquads:
            [x_c, y_c, w1, h1, w2, h2, w3, h3, w4, h4] = bquad
            [x1, y1, x2, y2, x3, y3, x4, y4] = [x_c+w1, y_c+h1, x_c+w2, y_c+h2, x_c+w3, y_c+h3, x_c+w4, y_c+h4]
...
def prepare_for_coco_bquad(predictions, dataset):
    import pycocotools.mask as mask_util
    import numpy as np
            ...
            [
                {
                    "image_id": original_id,
                    "category_id": mapped_labels[k],
                    "segmentation": [[bquad[0]+bquad[2], bquad[1]+bquad[3], bquad[0]+bquad[4], bquad[1]+bquad[5], bquad[0]+bquad[6], bquad[1]+bquad[7], bquad[0]+bquad[8], bquad[1]+bquad[9]]],
                    "bquad": [bquad],
                    "score": scores[k],
                }
                for k, bquad in enumerate(bquads)
            ]
        )
    return coco_results
    
def prepare_for_coco_bquad_pic(predictions, dataset):
    import pycocotools.mask as mask_util
    import numpy as np
    from pycocotools.coco import COCO
        ...
        if cfg.MODEL.BQUAD_ON:
            with tempfile.NamedTemporaryFile() as f:
                ...
                if output_folder:
                    file_path = os.path.join(output_folder, iou_type + ".json")
                res = evaluate_predictions_on_coco(
                    dataset.coco, coco_bquad_bbox_results, file_path, "bbox"
                )
                results.update(res)
    logger.info(results)
    check_expected_results(results, expected_results, expected_results_sigma_tol)
