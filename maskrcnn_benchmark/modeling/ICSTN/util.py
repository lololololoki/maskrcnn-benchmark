...
class Visdom():
	...
	def meanVar(self,opt,mean,var):
		...
		self.vis.image(mean[2].clip(0, 1), win="{0}_meanimage".format(opt.model), opts={"title": "{0} (TEST_mean_image)".format(opt.model)})
		self.vis.image(var[2].clip(0, 1), win="{0}_varimage".format(opt.model), opts={"title": "{0} (TEST_var_image)".format(opt.model)})
