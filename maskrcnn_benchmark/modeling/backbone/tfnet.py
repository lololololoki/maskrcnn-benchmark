...
class ResNet(nn.Module):
    ...
    def forward(self, x_pan, x_lr):
        out_pan1 = self.encoder1_pan(x_pan)
        out_pan2 = self.encoder2_pan(out_pan1)
        out_lr1 = self.encoder1_lr(x_lr)
        out_lr2 = self.encoder2_lr(out_lr1)

        out = torch.cat((out_lr2, out_pan2), dim=1)
        fusion1 = self.fusion1(out)
        fusion2 = self.fusion2(fusion1)

        restore1 = self.restore1(fusion2)
        bridge1 = self.bridge1(torch.cat((restore1, fusion1), dim=1))
        restore2 = self.restore2(bridge1)
        bridge2 = self.bridge2(torch.cat((restore2, out_pan1, out_lr1), dim=1))
        restore3 = self.restore3(bridge2)

        return out_pan2, out_lr2, fusion1, restore3


class Tfnet_res(nn.Module):
    def __init__(self):
        super(Tfnet_res, self).__init__()
...
class TFNet(nn.Module):
    ...
    def forward(self, x_pan, x_lr):
        ...
        restore1 = self.restore1(fusion2)
        #restore2 = self.restore2(torch.cat((restore1, fusion1),dim=1))
        #restore3 = self.restore3(torch.cat((restore2, encoder1_pan), dim=1))
