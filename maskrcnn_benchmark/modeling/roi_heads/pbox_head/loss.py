...
from maskrcnn_benchmark.modeling.pbox_coder import PboxCoder
...
class FastRCNNLossComputation(object):
    ...
    def __init__(self, proposal_matcher, fg_bg_sampler, box_coder, pbox_coder):
        """
        Arguments:
        ...
        self.pbox_coder = pbox_coder

    def match_targets_to_proposals(self, proposal, target):
        match_quality_matrix = boxlist_iou(target, proposal)
        matched_idxs = self.proposal_matcher(match_quality_matrix)
        # bquad RCNN needs "labels" and "bquad "fields for creating the targets
        target = target.copy_with_fields(["labels", "bquads"])
    ...
    def prepare_targets(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            bquads_per_image = self.pbox_coder.encode(
                matched_targets.get_field("bquads"), proposals_per_image.bbox
            )


            labels.append(labels_per_image)
            regression_targets.append(regression_targets_per_image)
            bquads.append(bquads_per_image)

        return labels, regression_targets, bquads
    ...
    def subsample(self, proposals, targets):
        ...
        labels, regression_targets, bquads = self.prepare_targets(proposals, targets)
    ...
    def __call__(self, class_logits=None, box_regression=None, bquad_regression=None, proposals=None, targets=None, tqr_loss_weights=None):
        ...
        if proposals is not None:
            labels, regression_targets, bquad_targets = self.prepare_targets(proposals, targets)

            # regression_targets = cat(regression_targets, dim=0)
            bquad_targets = cat(bquad_targets, dim=0)
            ...
            if_loss_not_limit = False
            if_PanMulImg_R_101 = True

            if if_loss_not_limit:
                cond = bquad_loss < 8
                bquad_loss = torch.where(cond, bquad_loss, (bquad_loss / 5) + 6.4)
            elif if_PanMulImg_R_101:
                cond = bquad_loss < 7
                bquad_loss = torch.where(cond, bquad_loss, (bquad_loss / 20) + 6.6)
            else:
...
def make_roi_box_loss_evaluator(cfg):
    ...
    pbox_coder = PboxCoder()

    fg_bg_sampler = BalancedPositiveNegativeSampler(
        cfg.MODEL.ROI_HEADS.BATCH_SIZE_PER_IMAGE, cfg.MODEL.ROI_HEADS.POSITIVE_FRACTION
    )

    loss_evaluator = FastRCNNLossComputation(matcher, fg_bg_sampler, box_coder, pbox_coder)
